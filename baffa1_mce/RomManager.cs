...
namespace baffa1_mce
{
    public class RomManager
    {
        ...
        public byte[] ExportRomDesc()
        {
            ASCIIEncoding _asiiencode = new ASCIIEncoding();
            MemoryStream binWriter;
            using (binWriter = new MemoryStream())
            {
            ...
            return binWriter.ToArray();
        }

        public byte[][] ExportRoms()
        {
            return ROMS;
        }
        ...
        public string CalculateAVgCyclesPerInstruction()
        {
            ...
            msg += "Cycles per Instructions\r\n";
            msg += "-------------------------\r\n";

            for (int i = 0; i < 256; i++)
            {
                int inst_count = instr_names[i].IndexOf("|") > -1 || instr_names[i].IndexOf("-") > -1 ? 2 : 1;
                int current_count = 0;
                ...
                if (i == 0)
                {
                    ...
                    msg += i.ToString("X2") + ": " + current_count.ToString() + "\t";
                    if (current_count > 0) valid_total++; else invalid_total++;
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        msg += "FD" + i.ToString("X2") + ": " + current_count.ToString() + "\r\n";
                        if (current_count > 0) valid_total++; else invalid_total++;
                    }
                }
            }

            average = ((double)count + 2) / valid_total; // the +2 accounts for fetch

            msg += "-------------------------\r\n";
            msg += "Valid Instr.: " + valid_total + "\r\n";
            msg += "Invalid Instr.: " + invalid_total + "\r\n";
            msg += "Average Cycles p/ Instr.: " + average.ToString("N2") + "\r\n";
